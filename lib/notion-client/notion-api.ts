...
export class NotionAPI {
    ...
    public async getPage(
        ...
        if (fetchCollections) {
            ...
            const func = async (collectionInstance: {
                collectionId: string;
                collectionViewId: string;
            }) => {
            ...
            try {
                ...
                if (signedUrls.length === allFileInstances.length) {
                    for (let i = 0; i < allFileInstances.length; ++i) {
                        ...
                        recordMap.signed_urls[file.permissionRecord.id] =
                            signedUrl;
                    }
                }
            } catch (err) {
    ...
    public async getCollectionData(
        ...
        if (groupBy) {
            ...
            const reducerLabel = isBoardType
                ? 'board_columns'
                : `${type}_groups`;
            loader = {
                type: 'reducer',
                reducers: {
                    [reducerLabel]: {
                        ...
                        groupSortPreference: groups.map(
                            (group) => group?.value,
                        ),
                        limit,
                    },
